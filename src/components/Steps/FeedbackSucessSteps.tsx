...
interface FeedBackSucessStepsProps {
  onFeedbackRestartRequested: () => void;
}

export const FeedBackSucessSteps = ({
  onFeedbackRestartRequested,
}: FeedBackSucessStepsProps) => {
  return (
    <>
      <header>
        <CloseButton />
      ...
      <div className="flex flex-col items-center py-10 w-[304px] ">
        ...
        <span className="text-xl m-3">Agradecemos o feedback!</span>
        <button
          onClick={onFeedbackRestartRequested}
          type="button"
          className="py-2 px-6  bg-zinc-800 rounded-md border-transparent text-sm leading-6 hover:bg-zinc-700 focus:outline:none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-900 focus:ring-brand-500 transition-colors disabled:opacity-50 disabled:hover:bg-b"
        >
          Quero enviar outro
        </button>
      </div>
